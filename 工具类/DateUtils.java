...
public class DateUtils {
	...
	public final static String FORMAT_YYYMMDDHHMMSS = "yyyy-MM-dd HH:mm:ss";
	...
	public static String subDataInterval(Date end, int interval) {
		...
		return format(calendar.getTime(), FORMAT_YYYMMDDHHMMSS);
	}
	...
	/**
	* 方法名: timeStamp13MakeUp
	* 作者/时间: dxwang-2020/7/16
	* 描述: 时间戳不够13位后面补0
	* 参数: getTime
	* 返回: java.util.Date
	* 异常场景: 
	*/
	public static  Date timeStamp13MakeUp(Long getTime){
		StringBuilder sb=new StringBuilder(getTime.toString());
		while(sb.length()<13){
			sb.append("0");
		}
		Calendar calendar = Calendar.getInstance();
		calendar.setTimeInMillis(Long.valueOf(sb.toString()));
		return calendar.getTime();
